 ...
 * ----------------------------------- */
  ...
  #define DPI "120"
#endif
static const char density[256] = DPI;	/*-D/-density arg for dvipng/convert*/
/* ---
...
mathtex_object_t *mathtex_object_ctor(request_rec *r) {
    ...
    conf->process_log_file_path = apr_pstrcat(r->pool, conf->home_dir, DIRECTORY_SEPARATOR, "process.log", NULL);
    o->process_log_file = fopen(conf->process_log_file_path, "a");
    if (o->process_log_file == NULL) {
        ap_log_perror(APLOG_MARK, APLOG_WARNING, 0, r->pool, 
                "failed to create process log file(%s).", 
                conf->process_log_file_path);
    }
    conf->math_log_file_path = apr_pstrcat(r->pool, conf->home_dir, DIRECTORY_SEPARATOR, "math.log", NULL);
    o->math_log_file = fopen(conf->math_log_file_path, "a");
    if (o->math_log_file == NULL) {
        ap_log_perror(APLOG_MARK, APLOG_WARNING, 0, r->pool, 
                "failed to create math log file(%s).", 
 ...
 * ======================================================================= */
/* --- entry point --- */
int mathtex_process(mathtex_object_t *o)
{
    request_rec *r = o->r;
    mathtex_config_t *conf = o->conf;
    char *query = o->query;
/* -------------------------------------------------------------------------
...
int  iscaching = 1;		/* true if caching images */
/* --- image rendering --- */
 ...
 *copyright2 =				/* second part of copyright */
 ...
 "+-----------------------------------------------------------------------+";
char	*usage2 =	/* second part of usage */
 "Example:\n"
...
if ( o->imagetype < 1 || o->imagetype > 2 ) o->imagetype = 1;   /* keep in bounds */
if ( o->imagemethod<1 || o->imagemethod>2 ) o->imagemethod = 1; /* keep in bounds */
/* ---
...
//*
ap_log_perror(APLOG_MARK, APLOG_DEBUG, 0, r->pool, "conf->latex_method: %s", conf->latex_method);
if ((apr_strnatcmp(conf->latex_method, "pdf") == 0 || apr_strnatcmp(conf->latex_method, "PDF") == 0) && conf->pdflatex != NULL) {
    o->latexmethod = 2;
    o->imagemethod = 2;
}
//*/
...
!=   NULL ) {				/* found \message directive */
  ...
  emitembedded(msgnumber,(isquery?2:0), o); /* emit image, suppress msgfp */
...
!=   NULL ) {				/* \convertpath{/path/to/convert} */
  strcpy(conf->convert,argstring);	/*interpret arg as /path/to/convert*/
  if ( strstr(conf->convert,"convert") == NULL ) { /* just path without name */
    if ( lastchar(conf->convert) != '/' )	/* not even a / at end of path */
      strcat(conf->convert,"/");		/* so add a / */
    strcat(conf->convert,"convert"); }	/* add name of convert program */
...
>=   1 ) o->imagetype = 2;			/* found -png so set png imagetype */
if ( strreplace(expression,"\\gif","",0,0) /* remove occurrences of \gif */
>=   1 ) o->imagetype = 1;			/* found -gif so set gif imagetype */
/* --- check for latex method directives (\latex or \pdflatex) --- */
if ( strreplace(expression,"\\latex","",1,0) >=   1 ) { /*remove occurrences of \latex*/
    o->latexmethod = 1;		/* found \latex so set latex method */
}
if ( strreplace(expression,"\\pdflatex","",0,0) >=   1 ) { /* remove \pdflatex */
    o->latexmethod = 2;		/* found \pdflatex so set pdflatex */
}
 ...
 * ------------------------------------------- */
if (getdirective(expression,"\\msglevel",1,0,1,argstring) != NULL)/*look for \msglvl*/ /* found \msglevel directive */
  o->conf->message_level = min2(atoi(argstring),MAXMSGLEVEL); /*assign new msglevel*/
...
if ( o->process_log_file!=NULL && o->conf->message_level>=1 ) {	/*copyright notice and path to image*/
  ...
  if ( o->conf->message_level >= 8 ) {		/* and if very verbose */
    fprintf(o->process_log_file,			/* timelimit info */
      ...
      (conf->timelimit != NULL?conf->timelimit:"none"));
    fflush(o->process_log_file); }			/* flush message buffer */
...
if ( md5hash != NULL ) {		/* md5str() almost surely succeeded*/
  ...
  if ( isempty(outfile) ) {	/* no explicit output file given */
    if ( !isdexists(conf->home_dir, r) ) { /* and no cache directory */
      if ( mkdir(conf->home_dir,perm_all) /* so make cache dir */
      !=   0 ) {			/* failed, emit embedded error image*/
        emitembedded(CACHEFAILED,isquery, o); /* emit CACHEFAILED message */
   ...
   * --------------------------------------- */
  if ( o->conf->message_level >= 1 )			/* check if logging */
    if ( iscaching )			/* only log images being cached */
      if ( isempty(outfile) )		/* and not saved to explicit outfile*/
        mathlog(hashexpr,md5hash, o);	/* log request for new image */
  /* ---
  ...
  ==   o->imagetype ) {			/* and emit cache if succeeded */
    ...
    emitembedded(msgnumber,isquery, o); }	/* emit message image */
  /* ---
 ...
 * ======================================================================= */
/* --- entry point --- */
int	mathtex (char *expression, char *filename, mathtex_object_t *o)
{
    request_rec *r = o->r;
    mathtex_config_t *conf = o->conf;
/* -------------------------------------------------------------------------
...
/* --- other variables --- */
//char    tempdir[256] = "\000";	/* temporary work directory */
char *tempdir = NULL;
...
//strninit(tempdir,filename,255);	/* maximum name length is 255 */
...
if ( !isdexists(tempdir/*filename*/, r) )	/* if temp directory doesn't exist */
  ...
  !=   0 ) {				/* mkdir failed */
      ap_log_rerror(APLOG_MARK, APLOG_WARNING, 0, r, "failed to mkdir(%s)", tempdir);
...
-------------------------------------------------------------------------- */
strcpy(latexfile,makepath("","latex",".tex", conf, r->pool)); /* latex filename latex.tex */
...
if ( conf->timelimit != NULL			/* given explict -DTIMELIMIT path */
...
&&   !iscompiletimelimit ) {		/* not using built-in timelimit() */
  ...
  strcat(command,makepath("",conf->timelimit,NULL, conf, r->pool)); /* timelimit program */
...
if ( o->latexmethod != 2 )			/* not explicitly using pdflatex */
  strcpy(subcommand,makepath("",conf->latex,NULL, conf, r->pool)); /* running latex program */
else					/* explicitly using pdflatex */
  strcpy(subcommand,makepath("",conf->pdflatex,NULL, conf, r->pool)); /* running pdflatex */
...
if ( o->latexmethod != 2 ) {			/* ran latex */
    char *latex_dvi = makepath("","latex",".dvi", conf, r->pool);
  if ( !isfexists(latex_dvi) ) { /* but no latex dvi */
    sys_stat = (-1);			/* signal that latex failed */
  }
}
if ( o->latexmethod == 2 ) {			/* ran pdflatex */
    char *latex_pdf = makepath("","latex",".pdf", conf, r->pool);
  if ( !isfexists(latex_pdf) ) { /* but no pdflatex pdf */
      ap_log_rerror(APLOG_MARK, APLOG_WARNING, 0, r, "%s not found.", latex_pdf);
...
if ( sys_stat == (-1) ) {		/* system() or pdf/latex failed */
  if ( !iserror ) {			/* don't recurse if errormsg fails */
    ...
    o->isdepth = ispicture = 0;		/* reset depth, picture mode */
    status = mathtex(errormsg, filename, o); /* recurse just once for error msg*/
...
-------------------------------------------------------------------------- */
if ( o->isdepth )				/* image info requested */
 if ( isfexists(makepath("","latex",".info", conf, r->pool)) ) { /* and have latex.info */
  FILE *info = fopen(makepath("","latex",".info", conf, r->pool),"r"); /*open it for read*/
...
if ( o->imagemethod == 1 ) {		/*dvipng method requested (default)*/
   ...
   *---------------------------------------------------------- */
  strcpy(command,makepath("",conf->dvipng,NULL, conf, r->pool)); /* running dvipng program */
...
-------------------------------------------------------------------------- */
if ( o->imagemethod == 2 ) {		/* dvips/convert method requested */
  /* ---
   * First run dvips to convert .dvi file to .ps postscript
   *------------------------------------------------------- */
  if ( o->latexmethod != 2 ) {		/* only if not using pdflatex */
    strcpy(command,makepath("",conf->dvips,NULL, conf, r->pool)); /* running dvips program */
    ...
    strcat(command,makepath("","latex",".dvi", conf, r->pool)); /* run dvips on latex.dvi */
    ...
    &&   ispicture ) {			/* and we ran dvips without -E */
      strcpy(command,makepath("",conf->ps2epsi,NULL, conf, r->pool)); /* running ps2epsi */
    ...
    ||   !isfexists(makepath("","dvips",".ps", conf, r->pool)) ) { /*dvips didn't create .ps*/
      msgnumber =			/* set corresponding message number*/
   ...
   *-------------------------------------------------- */
  strcpy(command,makepath("",conf->convert,NULL, conf, r->pool)); /*running convert program*/
  ...
  if ( o->latexmethod != 2 )		/* we ran latex and dvips */
    strcat(command,makepath("","dvips",".ps", conf, r->pool)); /* convert from postscript */
  if ( o->latexmethod == 2 )		/* we ran pdflatex */
    strcat(command,makepath("","latex",".pdf", conf, r->pool)); /* convert from pdf */
...
status = o->imagetype;			/* signal success */
/* -------------------------------------------------------------------------
Back to caller
-------------------------------------------------------------------------- */
end_of_job:
  ...
  if ( dir_stat >= 1 ) {
      rrmdir(tempdir/*filename*/, o); /*rm -r temp working dir*/
  }
 ...
 * ======================================================================= */
/* --- entry point --- */
int	mathlog ( char *expression, char *filename, mathtex_object_t *o)
{
    mathtex_config_t *conf = o->conf;
    request_rec *r = o->r;
/* -------------------------------------------------------------------------
...
-------------------------------------------------------------------------- */
if ( !isempty(conf->math_log_file_path) ) {		/* if a logfile is given */
    char *_filename = makepath(NULL,conf->math_log_file_path,NULL, conf, r->pool);
  if ( (filefp=fopen(_filename,"a")) /*open logfile*/
  !=   NULL ) {				/* ignore logging if can't open */
    int isreflogged = 0;		/* set true if http_referer logged */
    fprintf(filefp,"%s                 %s\n", /* timestamp, md5 file */
    timestamp(TZDELTA,0),		/* timestamp */
    makepath("",filename,extensions[o->imagetype]), conf, r->pool); /* hashed filename */
 ...
 * ======================================================================= */
/* --- entry point --- */
char	*makepath ( char *path, char *name, char *extension, mathtex_config_t *conf, apr_pool_t *pool)
{
/* -------------------------------------------------------------------------
Allocations and Declarations
-------------------------------------------------------------------------- */
...
if ( path == NULL ) {			/* use default path to cache */
    int home_dir_exists = FALSE;
    char *home_dir = apr_pstrcat(pool, conf->home_dir, DIRECTORY_SEPARATOR, CACHE, NULL);
    int res = access(home_dir, F_OK);
    if (res != 0) {
        apr_status_t status = apr_dir_make(home_dir, APR_FPROT_OS_DEFAULT, pool);
    ...
    if (home_dir_exists)		/* have a cache path */
      strcpy(namebuff,home_dir); /* begin filename with path */
}	
 ...
 * ======================================================================= */
/* --- entry point --- */
int	isdexists ( char *dirname, request_rec *r)
{
 ...
 * ======================================================================= */
/* --- entry point --- */
int	rrmdir ( char *path, mathtex_object_t *o)
{
/* -------------------------------------------------------------------------
Allocations and Declarations
-------------------------------------------------------------------------- */
...
-------------------------------------------------------------------------- */
if ( o->conf->message_level < 9 ) status = remove(path); /* remove file or directory */
end_of_job:
  return ( status );
 ...
 * ======================================================================= */
/* --- entry point --- */
int	emitcache (char *cachefile, int maxage, int isbuffer, mathtex_object_t *o)
{
    const int  isdepth = o->isdepth;
    request_rec *r = o->r;
/* -------------------------------------------------------------------------
Allocations and Declarations
-------------------------------------------------------------------------- */
...
{ /* --- emit mime content-type line --- */
    r->content_type = apr_pstrcat(r->pool, "Content-type: image/", extensions[o->imagetype], NULL);
}
/* -------------------------------------------------------------------------
emit bytes from cachefile
-------------------------------------------------------------------------- */
ap_rwrite(buffptr, nbytes, r);
end_of_job:
  return ( nbytes );			/* back with #bytes emitted */
...
int sig;
sigemptyset(&ss);
if (sigaddset(&ss, SIGALRM) != 0) return -1;
if (sigaddset(&ss, SIGCHLD) != 0) return -1;
if (sigaddset(&ss, SIGTERM) != 0) return -1;
if (sigaddset(&ss, SIGHUP) != 0) return -1;
if (sigaddset(&ss, SIGINT) != 0) return -1;
if (sigaddset(&ss, SIGQUIT) != 0) return -1;
if (sigprocmask(SIG_BLOCK, &ss, NULL) != 0) return -1;
 ...
 * ======================================================================= */
/* --- entry point --- */
char	*mathprep ( char *expression, mathtex_object_t *o)
{
/* -------------------------------------------------------------------------
Allocations and Declarations
-------------------------------------------------------------------------- */
 ...
 * ======================================================================= */
/* --- entry point --- */
int	emitembedded ( int imagenum, int isquery, mathtex_object_t *o)
{
    const int isdepth = o->isdepth;
    request_rec *r = o->r;
/* --------------------------------------------------------------------------
...
if ( isquery )				/* don't emit error to shell stdout*/
  if ( eimage != NULL ) {		/* have embedded image */
    o->imagetype = imgtype;		/* set embedded image type */
    nbytes = emitcache((char *)eimage,maxage,nbytes, o); } /* and emit image */
/* --- show message text on msgfp --- */
if ( isquery != 2 )			/* msgfp output enabled */
  if ( o->process_log_file!=NULL && o->conf->message_level>=1 )	/* display on msgfp, too */
    fprintf(o->process_log_file,"\nmathTeX> message#%d %s:\n         %s\n",
